use proptest::prelude::*;
use yrs::{Doc, GetString, ReadTxn, StateVector, Transact, Update};
use yrs::types::Text;
use yrs::updates::decoder::Decode;

mod common;

/// Text operation types for property-based testing
#[derive(Debug, Clone)]
enum TextOperation {
    Insert(usize, String),
    Delete(usize, u32),
}

impl TextOperation {
    /// Apply this operation to a Yrs document
    fn apply(&self, doc: &Doc) {
        let text = doc.get_or_insert_text("content");
        let mut txn = doc.transact_mut();

        match self {
            TextOperation::Insert(pos, s) => {
                let current_len = text.len(&txn);
                let safe_pos = (*pos).min(current_len as usize);
                text.insert(&mut txn, safe_pos as u32, s);
            }
            TextOperation::Delete(pos, len) => {
                let current_len = text.len(&txn);
                if *pos < current_len as usize {
                    let safe_len = (*len).min(current_len - *pos as u32);
                    if safe_len > 0 {
                        text.remove_range(&mut txn, *pos as u32, safe_len);
                    }
                }
            }
        }
    }
}

/// Strategy for generating text operations
fn text_operation() -> impl Strategy<Value = TextOperation> {
    prop_oneof![
        // Insert operations with positions 0-100 and short strings
        (0..100usize, "[a-z]{1,10}").prop_map(|(pos, s)| TextOperation::Insert(pos, s)),
        // Delete operations with positions 0-100 and lengths 1-10
        (0..100usize, 1..10u32).prop_map(|(pos, len)| TextOperation::Delete(pos, len)),
    ]
}

/// Strategy for generating sequences of operations
fn operation_sequence(min: usize, max: usize) -> impl Strategy<Value = Vec<TextOperation>> {
    prop::collection::vec(text_operation(), min..max)
}

/// Apply a sequence of operations to a document
fn apply_operations(doc: &Doc, ops: &[TextOperation]) {
    for op in ops {
        op.apply(doc);
    }
}

/// Get the current text content from a document
fn get_doc_content(doc: &Doc) -> String {
    let text = doc.get_or_insert_text("content");
    let txn = doc.transact();
    text.get_string(&txn)
}

/// Merge updates from doc_src into doc_dst
fn merge_updates(doc_dst: &Doc, doc_src: &Doc) {
    let txn_src = doc_src.transact();
    let update = txn_src.encode_state_as_update_v1(&StateVector::default());

    let mut txn_dst = doc_dst.transact_mut();
    txn_dst.apply_update(Update::decode_v1(&update).unwrap());
}

// ============================================================================
// Property-Based Tests
// ============================================================================

proptest! {
    #![proptest_config(ProptestConfig::with_cases(100))]

    /// Test that concurrent operations converge when applied in different orders
    /// Property: Merging the same set of updates in different orders yields same result
    #[test]
    fn prop_operations_commutative(
        ops_a in operation_sequence(1, 5),
        ops_b in operation_sequence(1, 5)
    ) {
        // Create two docs that will receive same updates in different orders
        let doc_a = Doc::new();
        let doc_b = Doc::new();

        // Apply operations to separate source docs
        let src_a = Doc::new();
        let src_b = Doc::new();
        apply_operations(&src_a, &ops_a);
        apply_operations(&src_b, &ops_b);

        // Merge in order: A then B
        merge_updates(&doc_a, &src_a);
        merge_updates(&doc_a, &src_b);

        // Merge in order: B then A
        merge_updates(&doc_b, &src_b);
        merge_updates(&doc_b, &src_a);

        // Both should converge to same state (CRDT commutativity)
        let content_a = get_doc_content(&doc_a);
        let content_b = get_doc_content(&doc_b);

        prop_assert_eq!(
            content_a,
            content_b,
            "CRDT should be commutative: merging updates in different orders should produce same result"
        );
    }
}

proptest! {
    #![proptest_config(ProptestConfig::with_cases(1000))]

    /// Test that re-applying the same updates is idempotent
    /// Property: Applying updates multiple times should not change the result
    #[test]
    fn prop_updates_idempotent(
        ops in operation_sequence(1, 20)
    ) {
        let doc = Doc::new();
        apply_operations(&doc, &ops);

        // Get state after first application
        let content1 = get_doc_content(&doc);

        // Encode the current state
        let txn = doc.transact();
        let update = txn.encode_state_as_update_v1(&StateVector::default());
        drop(txn);

        // Re-apply the same update
        let mut txn_mut = doc.transact_mut();
        txn_mut.apply_update(Update::decode_v1(&update).unwrap());
        drop(txn_mut);

        // Get state after re-application
        let content2 = get_doc_content(&doc);

        // Should be identical (idempotence)
        prop_assert_eq!(
            content1,
            content2,
            "Re-applying updates should be idempotent"
        );
    }
}

proptest! {
    #![proptest_config(ProptestConfig::with_cases(500))]

    /// Test convergence property: multiple docs with different ops should converge
    /// Property: All replicas that receive all updates should converge to same state
    #[test]
    fn prop_convergence_invariant(
        ops1 in operation_sequence(1, 15),
        ops2 in operation_sequence(1, 15),
        ops3 in operation_sequence(1, 15)
    ) {
        // Simulate 3 clients with different operation sequences
        let doc1 = Doc::new();
        let doc2 = Doc::new();
        let doc3 = Doc::new();

        apply_operations(&doc1, &ops1);
        apply_operations(&doc2, &ops2);
        apply_operations(&doc3, &ops3);

        // Create 3 final replicas that will receive all updates
        let replica_a = Doc::new();
        let replica_b = Doc::new();
        let replica_c = Doc::new();

        // Each replica receives all updates (in different orders)
        // Replica A: doc1 -> doc2 -> doc3
        merge_updates(&replica_a, &doc1);
        merge_updates(&replica_a, &doc2);
        merge_updates(&replica_a, &doc3);

        // Replica B: doc2 -> doc3 -> doc1
        merge_updates(&replica_b, &doc2);
        merge_updates(&replica_b, &doc3);
        merge_updates(&replica_b, &doc1);

        // Replica C: doc3 -> doc1 -> doc2
        merge_updates(&replica_c, &doc3);
        merge_updates(&replica_c, &doc1);
        merge_updates(&replica_c, &doc2);

        // All replicas should converge to same state
        let content_a = get_doc_content(&replica_a);
        let content_b = get_doc_content(&replica_b);
        let content_c = get_doc_content(&replica_c);

        prop_assert_eq!(
            &content_a,
            &content_b,
            "Replica A and B should converge"
        );

        prop_assert_eq!(
            &content_b,
            &content_c,
            "Replica B and C should converge"
        );

        prop_assert_eq!(
            &content_a,
            &content_c,
            "All replicas should converge to identical state regardless of merge order"
        );
    }
}
